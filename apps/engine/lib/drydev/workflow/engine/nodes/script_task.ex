defmodule DryDev.Workflow.Engine.Nodes.ScriptTask do
  ...
  def process(context) do
    ...
    DryDev.Workflow.Engine.Scripting.ScriptPool.execute_script(
      ref,
      node.script,
  ...
  def continue_after_wait(context) do
    ...
    case results do
      {:ok, outputs} when is_map(outputs) ->
        ...
        if first_output, do: NodeResult.next_with_params(first_output, merged), else: {:complete, %DryDev.Workflow.Engine.CompletionData.Blank{}}

      {:ok, _other} ->
        node = context.node
        first_output = List.first(node.outputs || [])
        if first_output, do: NodeResult.next(first_output), else: {:complete, %DryDev.Workflow.Engine.CompletionData.Blank{}}

      {:error, error} ->
        boundary = DryDev.Workflow.Engine.Nodes.Activity.get_boundary_event_to_exception(
