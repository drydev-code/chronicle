defmodule DryDev.Workflow.Engine.Diagrams.Definition do
  @moduledoc """
  ...
  def get_blank_start_event(%__MODULE__{nodes: nodes, start_events: starts}) do
    Enum.find_value(starts, fn id ->
      ...
      if match?(%DryDev.Workflow.Engine.Nodes.StartEvents.BlankStartEvent{}, node), do: node
    end)
  end

  def get_message_start_events(%__MODULE__{nodes: nodes, start_events: starts}) do
    Enum.filter(starts, fn id ->
      match?(%DryDev.Workflow.Engine.Nodes.StartEvents.MessageStartEvent{}, Map.get(nodes, id))
    end)
  ...
  def get_signal_start_events(%__MODULE__{nodes: nodes, start_events: starts}) do
    Enum.filter(starts, fn id ->
      match?(%DryDev.Workflow.Engine.Nodes.StartEvents.SignalStartEvent{}, Map.get(nodes, id))
    end)
    |> Enum.map(&Map.get(nodes, &1))
  end
  ...
  def get_timer_start_events(%__MODULE__{nodes: nodes, start_events: starts}) do
    Enum.filter(starts, fn id ->
      match?(%DryDev.Workflow.Engine.Nodes.StartEvents.TimerStartEvent{}, Map.get(nodes, id))
    end)
    |> Enum.map(&Map.get(nodes, &1))
  end
