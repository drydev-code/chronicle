defmodule DryDev.Workflow.Engine.Diagrams.DiagramStore do
  @moduledoc """
  ...
  def handle_call({:register, name, version, tenant, definition}, _from, state) do
    ...
    for msg_start <- DryDev.Workflow.Engine.Diagrams.Definition.get_message_start_events(definition) do
      :ets.insert(:diagram_message_starts, {{msg_start.message, tenant}, name})
    end

    # Register signal start events
    for sig_start <- DryDev.Workflow.Engine.Diagrams.Definition.get_signal_start_events(definition) do
