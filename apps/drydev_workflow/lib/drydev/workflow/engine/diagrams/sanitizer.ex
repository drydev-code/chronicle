defmodule DryDev.Workflow.Engine.Diagrams.Sanitizer do
  @moduledoc """
  ...
  defp check_gateway_defaults(findings, %{nodes: nodes}) do
    nodes
    |> Map.values()
    |> Enum.filter(fn node -> match?(%DryDev.Workflow.Engine.Nodes.Gateway{kind: :exclusive}, node) end)
    |> Enum.reduce(findings, fn node, acc ->
  ...
  defp is_activity?(node) do
    match?(%DryDev.Workflow.Engine.Nodes.ScriptTask{}, node) ||
    match?(%DryDev.Workflow.Engine.Nodes.ExternalTask{}, node) ||
    match?(%DryDev.Workflow.Engine.Nodes.CallActivity{}, node) ||
    match?(%DryDev.Workflow.Engine.Nodes.RulesTask{}, node)
  end
end
