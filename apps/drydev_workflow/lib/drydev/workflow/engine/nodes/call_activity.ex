defmodule DryDev.Workflow.Engine.Nodes.CallActivity do
  @moduledoc """
  ...
  defp handle_simulation(context) do
    ...
    case event do
      %DryDev.Workflow.Engine.PersistentData.CallStarted{} ->
        NodeResult.simulation_barrier_then_wait(:waiting_for_call)
      %DryDev.Workflow.Engine.PersistentData.CallCompleted{next_node: next} ->
        NodeResult.next(next)
      %DryDev.Workflow.Engine.PersistentData.CallCanceled{next_node: next} ->
        NodeResult.next(next)
      _ ->
        NodeResult.simulation_barrier_then_wait(:waiting_for_call)
    end
