defmodule DryDev.Workflow.Engine.Instance do
  @moduledoc """
  ...
  alias DryDev.Workflow.Engine.{Token, PersistentData}
  alias DryDev.Workflow.Engine.Diagrams.Definition
  alias DryDev.Workflow.Engine.Instance.{TokenState, EventReplayer, WaitRegistry, Migration}
  ...
  def handle_continue(:process_tokens, state) do
    state = DryDev.Workflow.Engine.TokenProcessor.process_active_tokens(state)
  ...
  defp complete_instance(state) do
    ...
    if DryDev.WorkflowServer.Host.LargeVariables.enabled?() do
      cleanup_state = state
      Task.start(fn ->
        all_params = cleanup_state.tokens |> Map.values() |> Enum.map(& &1.parameters)
        Enum.each(all_params, &DryDev.WorkflowServer.Host.LargeVariables.cleanup(&1, cleanup_state.tenant_id))
      end)
    end
  ...
  defp persist_completion(state) do
    Task.start(fn ->
      DryDev.Workflow.Persistence.EventStore.complete(
  ...
  defp persist_termination(state, reason) do
    Task.start(fn ->
      DryDev.Workflow.Persistence.EventStore.terminate(
