defmodule Chronicle.Server.Messaging.MessageConsumer do
  @moduledoc """
  Unified Broadway consumer for the workflow engine.
  Single queue bound to Chronicle.Realm.Any (topic exchange) with per-message-type
  routing keys, matching the Util.ServiceBus topology.

  Topology:
    Chronicle.Events (headers) → Chronicle.Realm.Any (topic) → queue (via routing key)
    Chronicle.{serviceName} (fanout) → queue (for commands)
  """
  use Broadway
  require Logger
  ...
  @default_queue "Chronicle.Engine"
  @default_realm_exchange "Chronicle.Realm.Any"
  @default_service_exchange "Chronicle.Engine"
  @message_assembly "Chronicle.Messages.Workflow"
  @max_retries 50
  @delay_bits 28
  ...
  def declare_topology(channel, realm_exchange, service_exchange) do
    # Declare the headers exchange (Chronicle.Events)
    AMQP.Exchange.declare(channel, "Chronicle.Events", :headers, durable: true)

    # Declare the realm topic exchange (Chronicle.Realm.Any)
    ...
    # Messages delayed via x-Chronicle.Delay binary tree arrive at x-Chronicle.Delivery
  ...
  defp republish_to_delay(data, metadata, retry_count, delay_seconds, exception) do
    ...
    AmqpConnection.publish("x-Chronicle.Delay", routing_key, data,
      headers: [
        {"Chronicle.CurrentRetry", to_string(retry_count)},
        {"Chronicle.DelayDestination", destination},
        {"Chronicle.LastException", Exception.message(exception)}
      ],
