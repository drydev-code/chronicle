defmodule DryDev.WorkflowServer.Host.Deployment.Manager do
  @moduledoc "Deployment management: upload, extract, register, validate."

  alias DryDev.Workflow.Engine.Diagrams.{DiagramStore, Parser, Sanitizer, ReferenceUpdater}
  ...
  defp extract_files(content, filename) do
    cond do
      String.ends_with?(filename, ".zip") ->
        DryDev.WorkflowServer.Host.Deployment.Package.extract(content)
  ...
  defp register_dmn(file, tenant_id) do
    ...
    DryDev.Workflow.Engine.Dmn.DmnStore.register(dmn_name, nil, tenant_id, file.content)
    {:ok, dmn_name}
  end
end
