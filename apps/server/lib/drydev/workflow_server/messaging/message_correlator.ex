defmodule DryDev.WorkflowServer.Messaging.MessageCorrelator do
  ...
  def correlate(tenant_id, message_name, business_key, payload \\ %{}) do
    ...
    process_names = DryDev.Workflow.Engine.Diagrams.DiagramStore.get_process_names_for_message(
      message_name, tenant_id
    )

    Enum.each(process_names, fn name ->
      case DryDev.Workflow.Engine.Diagrams.DiagramStore.get_latest(name, tenant_id) do
        {:ok, definition} ->
          params = %{
            id: UUID.uuid4(),
  ...
  def broadcast_signal(tenant_id, signal_name) do
    ...
    process_names = DryDev.Workflow.Engine.Diagrams.DiagramStore.get_process_names_for_signal(
