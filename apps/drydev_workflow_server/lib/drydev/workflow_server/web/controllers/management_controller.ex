defmodule DryDev.WorkflowServer.Web.Controllers.ManagementController do
  use Phoenix.Controller, formats: [:json]

  alias DryDev.Workflow.Engine.Instance
  alias DryDev.Workflow.Engine.Nodes.StartEvents.TimerStartEvent
  ...
  def timer_start_events(conn, _params) do
    defs = DryDev.WorkflowServer.Host.TelemetryService.get_process_definitions()
  ...
  def test_parse_diagram(conn, %{"filename" => filename}) do
    diagrams_dir = Application.app_dir(:drydev_workflow, "priv/diagrams")
    path = Path.join(diagrams_dir, filename)

    case File.read(path) do
      {:ok, content} ->
        case DryDev.Workflow.Engine.Diagrams.Parser.parse(content) do
          {:ok, definition} ->
            json(conn, %{
