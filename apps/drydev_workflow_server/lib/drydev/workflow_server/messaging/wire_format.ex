defmodule DryDev.WorkflowServer.Messaging.WireFormat do
  @moduledoc """
  ...
  - Realm headers: "DryDev.Realm.Any" => "True" for event routing
  """
  ...
  def encode(message_type, body, opts \\ []) do
    ...
    # Realm headers for the DryDev.Events headers exchange routing
    realm_headers = Map.new(realms, fn realm -> {"DryDev.Realm.#{realm}", "True"} end)

    headers = Map.merge(realm_headers, %{
      "DryDev.CorrelationId" => correlation_id
    })
  ...
  def decode(metadata, payload) when is_binary(payload) do
    ...
    %{
      message_type: message_type,
      correlation_id: Map.get(headers, "DryDev.CorrelationId") || Map.get(headers, "NServiceBus.CorrelationId"),
      body: body
    }
  end
